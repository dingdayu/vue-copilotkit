...
import { normalizeSuggestionTextForInsertion } from '../../lib/utils'
...
export const BaseCopilotTextarea = defineComponent({
  ...
  setup(props: BaseCopilotTextareaProps, { attrs, expose }) {
    ...
    const insertText = (autosuggestion: AutosuggestionState) => {
      const textBeforeCursor = editor.value?.state.doc.textBetween(0, autosuggestion.point, '\n', '\n') ?? ''
      const normalizedText = normalizeSuggestionTextForInsertion(autosuggestion.text, { textBeforeCursor })
      if (!normalizedText) {
        return
      }
      editor.value?.commands.insertContentAt(autosuggestion.point, normalizedText.replace(/\n/g, '<br>'))
    }
    ...
    useAddBrandingCss(suggestionStyleAugmented.value, props.disableBranding, props.brandingLabel)
