      ...
      >
        <HoveringInsertionPromptBoxCore
          ...
          performInsertion={async (insertedText: string) => {
            ...
            const textBeforeCursor = state.doc.textBetween(0, selection.value.from, '\n', '\n')
            const normalizedText = normalizeSuggestionTextForInsertion(insertedText, { textBeforeCursor })
            if (normalizedText) {
              props.editor.commands.insertContentAt(selection.value.from, normalizedText.replace(/\n/g, '<br>'))
            }
            // 如果文档变更有意义，则提交变更
            if (tr.docChanged) {
              view.dispatch(tr)
